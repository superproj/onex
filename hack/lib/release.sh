...
function onex::release::create_docker_images_for_server() {
  ...
  (
    ...
    # k8s.gcr.io is the constant tag in the docker archives, this is also the default for config hack in GKE.
...
#   - Cluster spin up/down hack and configs for various cloud providers
...
EOF
  ...
  cp -R "${ONEX_ROOT}/hack/release" "${release_stage}/"
...
EOF
  ...
  # Include hack/lib as a dependency for the cluster/ hack
  #mkdir -p "${release_stage}/hack"
  #cp -R "${ONEX_ROOT}/hack/lib" "${release_stage}/hack/"

  cp -R ${ONEX_ROOT}/{docs,configs,hack,deployments,init,README.md,LICENSE} "${release_stage}/"

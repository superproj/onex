...
source "${ONEX_ROOT}/hack/lib/protoc.sh"
cd "${ONEX_ROOT}"
...
function codegen::protobuf() {
    ...
    if onex::protoc::check_protoc >/dev/null; then
      ...
      onex::log::status "protoc ${PROTOC_VERSION} not found (can install with hack/install-protoc.sh); generating containerized..."
...
function codegen::clients() {
    ...
    ${ONEX_ROOT}/hack/fix-generated-client.sh

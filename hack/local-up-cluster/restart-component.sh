...
# Usage: `hack/local-up-cluster/restart-component.sh kube-scheduler`
...
if [[ "${command}" == "" ]];then
  onex::log::error "Cannot run hack/local-up-cluster/restart-component.sh with empty component name"
  exit 1
fi

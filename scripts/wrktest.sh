...
2. 执行命令： ./wrktest.sh diff onex.dat http.dat

> Note: 需要确保系统安装了 wrk 和 gnuplot 工具
EOF

onexroot="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
wrkdir="${onexroot}/_output/wrk"
jobname="onex"
duration="300s"
threads=$((3 * `grep -c processor /proc/cpuinfo`))

source "${onexroot}/scripts/lib/color.sh"
...
OPTIONS:
  -h                     Usage information
  -n                     Performance test task name, default: onex
...
function onex::wrk::convert_plot_data()
{
...
function onex::wrk::prepare()
{
  rm -f ${wrkdir}/${datfile}
}

# Plot according to gunplot data file
function onex::wrk::plot() {
  gnuplot <<  EOF
...
onex::wrk::start_performance_test() {
  onex::wrk::prepare
  ...
  do
    ...
    result=`eval ${wrkcmd}`
    onex::wrk::convert_plot_data "${result}"
  done

  echo -e "\nNow plot according to ${C_MAGENTA}${wrkdir}/${datfile}${C_NORMAL}"
  onex::wrk::plot &> /dev/null
  echo -e "QPS graphic file is: ${C_MAGENTA}${wrkdir}/${qpsttlb}${C_NORMAL}
Success rate graphic file is: ${C_MAGENTA}${wrkdir}/${successrate}${C_NORMAL}"
}
...
case $1 in
  "diff")
    ...
    onex::wrk::plot_diff `basename $2` `basename $3`
    exit 0
    ;;
  *)
    if [ "$#" -lt 1 ];then
    ...
    onex::wrk::setup
    onex::wrk::start_performance_test "${url}"
    ;;
esac
